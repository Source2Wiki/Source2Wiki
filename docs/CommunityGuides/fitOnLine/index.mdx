---
title: Modular pipes with FitOnLine
...
![Procedural Pipe Overview](./assets/fitonline-hero-demo.webp)
...
:::note
This guide builds on the fundamentals in the [SmartProps guide](../smartProps/index.mdx). Start there when working with SmartProps for the first time.
:::
...
1. Pivot Placement: Place the origin pivot at the start or end border of the mesh along the connection axis, such as Z or Y.
   ...
   ![Pivot Requirement](./assets/fitonline-pivot-requirement.png)
...
2. Subdivision Density for Bending: Straight pipe sections only need a few longitudinal segments. Meshes intended for curvature or deformation require extra edge loops along their length so they deform smoothly when passed through `BendDeformer`.
   ...
   ![Segment Density](./assets/fitonline-segment-density.png)
...
3. Grid-Aligned Sizing: Use predictable unit lengths such as 32, 64, 128, or 192 units. These values keep the length and selection criteria calculations direct.
   ...
   ![Clean Dimensions](./assets/fitonline-clean-dimensions.png)
...
![Blender Mesh Set](./assets/fitonline-blender-mesh-set.png)
...
3. Compile the models in the <Game name="cs2"/> tools.

:::tip
Configuring each `.vmdl` by hand is slow for a kit this size.
<Tool name="hammer5tools"/> batch-generates and synchronizes the whole set from one reference
asset — see [AssetGroup Maker](../../ExternalTools/Hammer5Tools/editors/asset-group.mdx#batch-import-a-mesh-kit).
:::
...
2. Import all `.vmdl` files into the SmartProp hierarchy. <Tool name="hammer5tools"/> has a Bulk Model Importer for this:
   ...
   ![Bulk Model Importer](./assets/fitonline-bulk-model-importer.png)

3. Move all models except `pipe_01_64_a` into a temporary group and disable that group to focus on the base piece:

   ![Temporary Group](./assets/fitonline-hierarchy-disabled-group.png)
...
5. On the `pipe_01_64_a` model element, add a Selection Criteria: LinearLength modifier and set Length to `64`.
   ...
   ![LinearLength Setup](./assets/fitonline-basic-setup-linearlength.png)
...
6. Place the SmartProp in Hammer to test it. Because the `FitOnLine` length is 128 and each pipe segment is 64, exactly 2 segments are placed:
   ...
   ![First Hammer Test](./assets/fitonline-hammer-first-test.png)
...
9. In the CreateSizer modifier:
   ...
   ![CreateSizer Setup](./assets/fitonline-create-sizer-modifier.png)

   ![Sizer Bounds](./assets/fitonline-sizer-bounds-config.png)
...
11. In the Hammer viewport (`Shift + S`), drag the sizer arrow handle to dynamically lengthen the pipe:
    ...
    ![Bind Sizer to End](./assets/fitonline-bind-sizer-to-end.png)
...
14. In the model's Selection Criteria: LinearLength, set Allow Scale to `true`, Min Length to `64`, and Max Length to `192`.
    ...
    ![Scale Mode LinearScale](./assets/fitonline-scale-mode-linearscale.png)
...
15. To prevent stretched textures when scaling pipes, enable UV scaling in the Source 2 material (`.vmat`):
    ...
    ![Material UV Scale](./assets/fitonline-material-uv-scale.png)
...
16. Adding End Caps:
    ...
    ![Add Caps Translation](./assets/fitonline-add-caps-translation.png)
...
3. Move the translation and a `CreateRotator` modifier to the End `PickOne` element.
   ...
   ![Nested End PickOne](./assets/fitonline-nested-end-pickone.png)
...
:::
   ...
   ![Nested Self Reference](./assets/fitonline-nested-self-reference.png)

5. Customize handle shapes, colors, and offset positions for the PickOne widgets:

   ![Handle Offsets & Colors](./assets/fitonline-handle-offsets-colors.png)
...
6. Configuring Branch Connectors:
   ...
   ![Branching 32b Offset](./assets/fitonline-branching-32b-offset.png)

7. Repeat the same setup for 90-degree corner connectors (`pipe_01_crn_a` and `pipe_01_crn_b`):

   ![Corner Elements](./assets/fitonline-corner-elements.png)
...
3. Attach a CreateRotator modifier to the `Bend` group:
   ...
   ![Bend Rotator Setup](./assets/fitonline-bend-rotator-setup.png)
...
4. Reorganize and clean up the hierarchy:
   ...
   ![Hierarchy Cleanup](./assets/fitonline-hierarchy-cleanup.png)
...
5. Configuring BendDeformer:
   ...
   ![BendDeformer Final](./assets/fitonline-bend-deformer-final.png)
...
![Final Pipe Demo](./assets/fitonline-hero-demo.webp)
