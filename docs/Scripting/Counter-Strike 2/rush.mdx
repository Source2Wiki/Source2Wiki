...
Starting the mode executes `cfg/gamemode_rush.cfg`, and offline games additionally run `gamemode_rush_offline.cfg`, which raises `bot_quota` to 6. The settings that matter most for Rush, described in full in the [console variable list](../../Convars/index.mdx):
...
| `bot_quota` | `2`, with `bot_quota_mode fill` |

The config also sets `mp_default_team_winner_no_objective -1` with the comment `// 2 == CTs, 3 == Ts`. That comment is wrong, the engine treats `3` as a CT win and `2` as a T win.
...
At every live round start the script sets `mp_ignore_round_win_conditions 1`.
