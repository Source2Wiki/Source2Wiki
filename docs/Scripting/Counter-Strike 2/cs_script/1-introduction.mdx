...
:::info
`cs_script_demo.vmap` is a Valve made map to demonstrate basic functionality of the system, it can be found in 
`Counter-Strike Global Offensive\content\csgo_addons\cs_script_demo\maps\cs_script_demo.vmap`

A significant amount of documentation on this page is taken from `Counter-Strike Global Offensive\content\csgo_addons\cs_script_demo\maps\scripts\point_script.d.ts`
:::

`Counter-Strike Global Offensive\content\csgo_addons\cs_script_demo\maps\scripts\point_script.d.ts` documents the JavaScript API for cs_script scripts attached to point_script entities.
...
:::note
There are projects offering full <Tool name="ts"/> wrappers over the current JavaScript API, such as [cs_script_boilerplate](https://github.com/Source2ZE/cs_script_boilerplate) however
...
- Create a point_script entity in your map and set its cs_script field to reference your JavaScript file as a vjs asset.
     - See `Counter-Strike Global Offensive\content\csgo_addons\cs_script_demo\maps\cs_script_demo.vmap`. There is a point_script entity in there named "hello_cs_script" that runs `hello.js`. There are a handful of other examples as well.
