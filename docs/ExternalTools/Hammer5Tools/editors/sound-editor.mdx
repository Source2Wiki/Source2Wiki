...
With <Game name="cs2"/> running on the configured NetConsole port, the editor can send `snd_sos_stop_all_soundevents` followed by `snd_sos_start_soundevent <name>` to preview the selected event.
